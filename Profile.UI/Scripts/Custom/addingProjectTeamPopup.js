...
function addPopup(container, linkList, linkSend, linkMenu) {
    ...
    function viewListForSelect(dat, item) {
        ...
        var checkedTrainee = checkTrainee(dat.Trainees, item);

        var optListTrainee = dat.Trainees.map(function (it, ind) {

            return "<option class='mainList' value='" + it.id + "'>" + it.name + "</option>"
        });

        var optListMentor = dat.Mentors.map(function (it, ind) {
    ...
    function specBinder() {        
        ...
        $(document).on("change", popupWrapper + " .spec select", function () {
            ...
            getData(linkList + spec, function (d) {
    ...
    function showNewTeam(d) {
        ...
        preloadAfter("#save")
        hidePopup();
        getData(linkMenu + "?teamId=" + d, ProjectsList.addProject);
        location.hash = "#id=" + d;
        //location.hash = "#id-" + d;
    }
    ...
    function postData() {
        ...
        if (valid) {
            ...
            sendData(linkSend, newItem, showNewTeam);
        }
        else {
        ...
        d.map(function (item, ind) {            
            ...
            if(cond.length){
                ...
                cond.each(function (indx, it) {
                    ...
                    //console.log(item.name,value,ind)
